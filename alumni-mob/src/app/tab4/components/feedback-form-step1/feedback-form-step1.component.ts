...
export class FeedbackFormStep1Component implements OnInit {
  ...
  async openStep2Modal() {
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        // ici on passe une propriété qu'on peut récuperer par la prochaine modal
  ...
  async openStep3Modal() {
    ...
    const dataForStep3Modal = {
      ...
      // internOccupation: this.intern[0].occupation, // Temporarily disabled for the final project presentation
      internCompany: this.intern[0].company.name,
      internId: this.intern[0]._id,
    };
    console.log(`dataForStep3Modal: ${JSON.stringify(dataForStep3Modal)}`);
    ...
    if (this.isCompanySelected === false) {
      const modal = await this.modalCtrl.create({
        ...
        backdropDismiss: false,
        id: newModalId,
        componentProps: dataForStep3Modal,
      });
      this._feedbackFormModals.modalIds.push(newModalId);
      modal.present();
